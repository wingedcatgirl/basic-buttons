...
To access the API, you can either add Basic Buttons as a dependency for your mod, or you can clone `buttons.lua` from this repository and load it unmodified. If you pick the latter option, please add Basic Buttons with its current version number to the "provides" list in your mod's metadata. ([Example](https://github.com/wingedcatgirl/Menthol/blob/jokerbuttons/Menthol.json#L15)) You may also benefit from adding `lsp_defs.lua` to your workspace; it's not required for function, but it lets your Lua Language Server provide hints and validation.
